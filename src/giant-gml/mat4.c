...
inline
void mat4_rotate(mat4_t* restrict mat, vec3_t vec, float angle)
{
    mat4_identity_init(mat);

    vec3_t vec_normal;
    vec3_copy(vec, &vec_normal);
...
void mat4_lookat(mat4_t* restrict mat, vec3_t pos, vec3_t direction)
{
    ...
    vec3_copy(direction, &dir_normal);
    vec3_normalize(&dir_normal);

    vec3_cross_to(dir_normal, (vec3_t){0.0f, 1.0f, 0.0f}, &side);
    vec3_normalize(&side);

    vec3_cross_to(side, dir_normal, &up);
    vec3_normalize(&up);
    ...
    (*mat)[3][0] = -vec3_dot(side, pos);
    (*mat)[3][1] = -vec3_dot(up, pos);
    (*mat)[3][2] = vec3_dot(dir_normal, pos);
}
